...
import { Upload, FileText, Download, AlertCircle, CheckCircle, User, Calendar, Activity, Type } from 'lucide-react';
...
const MedicalReportApp = () => {
  const [reportInput, setReportInput] = useState({ text: "", file: null });
  ...
  const handleSubmit = async ({ text, file }) => {
    setIsLoading(true);
    setError(null);
    console.log("sending this data :", text); 
    try {
  ...
  <div className="mt-6 space-y-6">
    ...
    <div>
      ...
      {Object.entries(currentReport.summary).map(([section, sectionData]) => (
        <div key={section} className="bg-white rounded-xl shadow p-4 mb-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border border-gray-200 rounded-md">
              ...
              <tbody>
                {Object.entries(sectionData.details).map(([test, result]) => (
